...
static char sort_buf[SORT_NUM] = { 'b', 'c', 'f', 's', '\0' };
...
static int array_auto_helper(int action, int i0, int i1)
{
    ...
    while (cont) {
        ...
        case HELPER_PROBE_ARRAY:
            ...
            if ((action & HELPER_ACTION_MASK) == HELPER_ACTION_ADD)
                insert_to_array(probe0, true, true, true, faketime); // TODO: Check bool flags
            else
        ...
        case HELPER_AUTH_ENTRY:
            ...
            if ((action & HELPER_ACTION_MASK) == HELPER_ACTION_ADD)
                insert_to_denied_req_array(auth_entry0, true, faketime); // TODO: Check bool flags
            else
    ...
    while (curr_arg < argc && ret == 0)
    {
        ...
        else if (strcmp(*argv, "faketime") == 0)
        {
            ...
            if (curr_arg + args_required <= argc)
            {
                ...
                {
                    printf("FAKETIME \"%s\": Unknown - stopping!\n", *(argv + 1));
                    ret = -1;
                }
            }
        }
        ...
        else if (strcmp(*argv, "client") == 0)
        {
            ...
            while (ret == 0 && curr_arg + args_required < argc)
            {
                ...
                else if (!strncmp(fn, "ht=", 3)) load_u8(&cl0.ht, fn + 3);
                else if (!strncmp(fn, "vht=", 4)) load_u8(&cl0.vht, fn + 4);
            ...
            if (ret == 0)
            {
                insert_client_to_array(cl0, faketime);
            }
        }
        else if (strcmp(*argv, "probe") == 0)
        {
            ...
            if (ret == 0)
            {
                insert_to_array(pr0, true, true, true, faketime);
            }
        }
        else if (strcmp(*argv, "auth") == 0)
        {
            ...
            if (ret == 0)
            {
                insert_to_denied_req_array(au0, true, faketime);
            }
        }
        else if (strcmp(*argv, "dawn") == 0) // Load metrics that configure DAWN
        {
            ...
            while (ret == 0 && curr_arg + args_required < argc)
            {
                ...
                if (false);  // Hack to allow easy paste of generated code
                    ...
                    printf("ERROR: Loading DAWN, but don't recognise assignment \"%s\"\n", fn);
                    ret = 1;
                }
        ...
        else if (strcmp(*argv, "eval_probe_metric") == 0)
        {
            ...
            if (curr_arg + args_required <= argc)
            {
                ...
                memcpy(&test_mac.unpacked.u8[0], probe0.bssid_addr, sizeof(probe0.bssid_addr));
                ...
                {
                    int tr = eval_probe_metric(probe0);
